...
export async function callBitrix<T>(
  ...
  try {
    ...
    throw new BitrixApiError('INVALID_RESPONSE', 'Failed to parse Bitrix API response')
  }
  ...
  if (payload.result === undefined) {
    throw new BitrixApiError('EMPTY_RESULT', 'Bitrix API returned an empty result')
  }
...
export async function getSpaType(webhookUrl: string, id: number): Promise<CrmType> {
  const result = await callBitrix<{ type: CrmType }>(webhookUrl, 'crm.type.get', { id })
  return result.type
}
...
export async function ensureWorkplace(
  ...
  try {
    const sourceWorkplace = await getAutomatedSolution(sourceWebhook, sourceCustomSectionId)
  ...
  const match = existing.find(
    (workplace) => workplace.title.toLowerCase() === workplaceTitle.toLowerCase(),
  )
...
export function buildSpaCreateFields(sourceType: CrmType, customSectionId: number | null) {
...
export async function createSpaType(
  ...
  const result = await callBitrix<{ type: CrmType }>(webhookUrl, 'crm.type.add', {
...
export async function fetchCustomFields(
  ...
  const result = await callBitrix<{ fields: Record<string, CrmFieldDescription> }>(
    webhookUrl,
    'crm.item.fields',
    {
      entityTypeId,
  ...
  for (const [fieldCode, fieldData] of Object.entries(result.fields ?? {})) {
    ...
    const title = fieldData.title ?? fieldCode.replace(/^uf/, '').replaceAll('_', ' ')
...
export function rebuildFieldName(upperName: string, spaNumericId: number): string {
  ...
  const suffix = source.replace(/^UF_CRM_\d+_?/i, '').replace(/[^A-Z0-9_]/g, '_')
